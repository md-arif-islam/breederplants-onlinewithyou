...
<section class="content-main">
    <div class="content-header">
        <h2 class="content-title">Edit Variety Report</h2>
    </div>
    <div class="card card-body">
        <form action="{{ route('variety-reports.update', $varietyReport->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-6">
                    <div class="mb-4">
                        ...
                        <input type="text" name="variety_name"
                               ...
                               value="{{ old('variety_name', $varietyReport->variety_name) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <select name="grower_id" class="form-select @error('grower_id') is-invalid @enderror" id="grower_id">
                            ...
                            @foreach($growers as $user)
                                @if($user->grower)
                                    <option value="{{ $user->id }}" {{ $varietyReport->grower_id == $user->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="breeder_id" class="form-select @error('breeder_id') is-invalid @enderror" id="breeder_id">
                            ...
                            @foreach($breeders as $user)
                                @if($user->breeder)
                                    <option value="{{ $user->id }}" {{ $varietyReport->breeder_id == $user->id ? 'selected' : '' }}>
                                        {{ $user->breeder->company_name }}
                                    </option>
                                @endif
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="date_of_propagation"
                               class="form-control @error('date_of_propagation') is-invalid @enderror" id="date_of_propagation"
                               value="{{ old('date_of_propagation', $varietyReport->date_of_propagation) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="date_of_potting"
                               class="form-control @error('date_of_potting') is-invalid @enderror" id="date_of_potting"
                               value="{{ old('date_of_potting', $varietyReport->date_of_potting) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="amount_of_plants"
                               class="form-control @error('amount_of_plants') is-invalid @enderror" id="amount_of_plants"
                               value="{{ old('amount_of_plants', $varietyReport->amount_of_plants) }}"
                ...
                <div class="col-md-6">
                    <div class="mb-4">
                        ...
                        <div id="samples-schedule-container">
                            @php
                                $samplesSchedule = old('samples_schedule', json_decode($varietyReport->samples_schedule ?? '[]', true));
                            @endphp
                            @foreach($samplesSchedule as $index => $date)
                                <div class="input-group mb-2" id="sample-schedule-{{ $index }}">
                                    <input type="date" name="samples_schedule[]"
                                           class="form-control @error('samples_schedule.' . $index) is-invalid @enderror"
                                           value="{{ $date }}">
                                    <button type="button" class="btn btn-danger" onclick="removeSampleSchedule({{ $index }})">Remove</button>
                                    @error('samples_schedule.' . $index)
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            @endforeach
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="pot_size"
                               ...
                               value="{{ old('pot_size', $varietyReport->pot_size) }}" placeholder="Enter pot size">
                        @error('pot_size')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="pot_trial" class="form-select @error('pot_trial') is-invalid @enderror" id="pot_trial">
                            <option value="1" {{ old('pot_trial', $varietyReport->pot_trial) == 1 ? 'selected' : '' }}>Yes</option>
                            <option value="0" {{ old('pot_trial', $varietyReport->pot_trial) == 0 ? 'selected' : '' }}>No</option>
                        </select>
                        @error('pot_trial')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <select name="open_field_trial" class="form-select @error('open_field_trial') is-invalid @enderror" id="open_field_trial">
                            <option value="1" {{ old('open_field_trial', $varietyReport->open_field_trial) == 1 ? 'selected' : '' }}>Yes</option>
                            <option value="0" {{ old('open_field_trial', $varietyReport->open_field_trial) == 0 ? 'selected' : '' }}>No</option>
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="status" class="form-select @error('status') is-invalid @enderror" id="status">
                            <option value="1" {{ old('status', $varietyReport->status) == 1 ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ old('status', $varietyReport->status) == 0 ? 'selected' : '' }}>Inactive</option>
                        </select>
                        @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
...
@section('scripts')
<script>
    let sampleScheduleIndex = {{ count($samplesSchedule) }};

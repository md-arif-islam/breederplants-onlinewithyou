        ...
        <div class="card card-body mb-4">
            ...
            <form action="{{ route('growers.update', $grower->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" class="form-control" id="name" value="{{ $grower->name }}" placeholder="Enter name">
                </div>
                <div class="mb-4">
                    <label for="email" class="form-label">Email</label>
                    <input type="email" name="email" class="form-control" id="email" value="{{ $grower->email }}" placeholder="Enter email">
                </div>
                <div class="mb-4">
                    <label for="status" class="form-label">Status</label>
                    <select name="status" class="form-select" id="status">
                        <option value="active" {{ $grower->status == 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ $grower->status == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
                <div class="d-grid">
